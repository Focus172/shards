...
fn is_thompson_shell_payload(p: &str, n: usize) -> bool {
    let mut has_lower = false;
    let mut ret = false;
    p.chars().take(n).for_each(|c| {
        if c.is_lowercase() || c == '$' || c == '`' {
            has_lower = true;
        }
        if c == '\n' && has_lower {
            ret = true;
...
fn is_thompson_shell_script(path: &str) -> bool {
    // Paths ending in ".zigi" or ".rsi" are never considered Thompson shell scripts.
    let ext = path.to_string().split('.').last().unwrap();
    if ext == "zigi" || ext == "rsi" {
        return false;
    }
    ...
    if let Ok(mut f) = file {
        ...
        return read >= 0 && is_thompson_shell_payload(&str, read);
    } else {
        return false;
    }
}
...
fn safe_launch_process(p: Process, actual_cmd: &str, cargv: &[&str], cenvv: &[&str]) -> ! {
    drop(p);

    // This function never returns, so we take certain liberties with constness.
    let envv = cenvv.clone();
    let argv = cargv.clone();
    ...
    let errno = match res {
        Ok(_) => unreachable!("How did you even get here?"),
        Err(errno) => errno,
    };
    ...
    if errno == Errno::ENOEXEC && is_thompson_shell_script(actual_cmd) {
        // Construct new argv.
